...
<html lang="en">
    ...
    <head>
        ...
        <title>UMC-Fire NOC | Business List</title>
                                ...
                                <div class="card">
                                    <div class="card-body" style="border: 1px solid rgb(3, 155, 155);">
                                        <div class="col-md-12 col-sm-12 text-left">
                                            <a class="btn btn-primary" style="float:right;" href="{{ route('business.create') }}" role="button">
                                                <b>+ &nbsp; Add Business</b>
                                            </a>
                                        </div>
                                        <h4 class="card-header text-primary">All Business List</h4>
                                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tbody>
                                                @foreach ($data as $key => $value)
                                                    <tr>
                                                        <td>{{ $key + 1 }}</td>
                                                        <td>{{ $value->business_nature }}</td>
                                                        <td>
                                                            <a href="{{ route('business.show', $value->id) }}" class="btn btn-primary btn-sm">
                                                                <b><i class="mdi mdi-eye-circle-outline"> View</i></b>
                                                            </a>
                                                        </td>

                                                        <td>
                                                            <a href="{{ route('business.edit', $value->id) }}" class="btn btn-warning btn-sm text-dark">
                                                                <b><i class="mdi mdi-account-edit"> Edit</i></b>
                                                            </a>
                                                        </td>

                                                        <td>
                                                            <form action="{{ route('business.destroy', $value->id) }}" method="post">
                                                                @csrf
                                                                @method('DELETE')

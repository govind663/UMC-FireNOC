                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body" style="border: 1px solid rgb(3, 155, 155);">
                                        ...
                                        @if($status == 0)
                                        <h4 class="card-header text-primary">All Pending New Other NOC List</h4>
                                        @elseif($status == 1)
                                        <h4 class="card-header text-primary">All Unpaid New Other NOC List</h4>
                                        @elseif($status == 2)
                                        <h4 class="card-header text-primary">All Paid New Other NOC List</h4>
                                        @elseif($status == 3)
                                        ...
                                        @elseif($status == 5)
                                        <h4 class="card-header text-primary">All Underprocess New Other NOC List</h4>
                                        @elseif($status == 6)
                                        <h4 class="card-header text-primary">All Reviewed New Other NOC List</h4>
                                        @elseif($status == 7)
                                        <h4 class="card-header text-primary">All Invoice Generated New Other NOC List</h4>
                                        @endif
                                            ...
                                            <tbody>
                                                @foreach ($data as $key => $value)
                                                    <tr>
                                                        ...
                                                        <td><span class="bg-success text-light p-1">Approved</span></td>
                                                        ...
                                                        <td><span class="bg-danger text-light p-1">Reviewed</span></td>
                                                        @elseif ($value->status == 7)
                                                        <td><span class="bg-primary text-light p-1">Invoice Generated Successfully</span></td>
                                                        @endif
                                                        ...
                                                        <td style="display:flex;">
                                                            <a href='{{ url("/new_other_noc/show/{$value->NC_NOC_ID}/{$value->status}") }}' class="btn btn-primary btn-sm">
                                                                <b><i class="mdi mdi-eye-circle-outline"> View</i></b>
                                                            </a>
                                                            &nbsp;&nbsp;
                                                            <a href='{{ url("/download_new_other_noc_pdf/{$value->NC_NOC_ID}/{$value->status}") }}' class="btn btn-dark btn-sm ">
                                                                <b><i class="mdi mdi-file"> Download View </i></b>
                                                            </a>

                                                            @if ($value->status == 0 || $value->status == 4)
                                                            &nbsp;&nbsp;
                                                            <a href='{{ url("/new_other_noc/edit/{$value->NC_NOC_ID}/{$value->status}") }}' class="btn btn-warning btn-sm text-dark">
                                                                <b><i class="mdi mdi-account-edit"> Edit</i></b>
                                                            </a>
                                                            &nbsp;&nbsp;

                                                            <form action='{{ url("/new_other_noc/delete/{$value->NC_NOC_ID}/{$value->d_ID}/{$value->status}") }}' method="post">
                                                                @csrf
                                                                @method('DELETE')
                                                            ...
                                                            &nbsp;&nbsp;
                                                            {{-- @if ($value->status == 1)
                                                            <a href='{{ url("/make_payment/create/{$value->NC_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' class="btn btn-success btn-sm ">
                                                            ...
                                                            <a href='{{ url("/invoice/{$value->NC_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' class="btn btn-dark btn-sm ">
                                                            ...
                                                            &nbsp;&nbsp;
                                                            @if ($value->status == 3 )
                                                            <a href='{{ url("/certificate/{$value->NC_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' class="btn btn-warning btn-sm text-dark">
                                                            ...
                                                            <button type="button" class="btn btn-warning text-dark btn-sm waves-effect waves-light" data-bs-toggle="modal" data-bs-target=".bs-example-modal-lg_{{ $value->NC_NOC_ID }}">Upload Payment Recepit</button>
                                                            @endif
                                                    ...
                                                    {{-- Payment Receipt --}}
                                                    <div class="modal fade bs-example-modal-lg_{{ $value->NC_NOC_ID }}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog modal-lg">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body">
                                                                    <div class="row">
                                                                        <form class="auth-input p-4"  method="POST" action='{{ url("/upload_payment_receipt/{$value->NC_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' enctype="multipart/form-data" autocomplete="off" >
                                                                            @csrf

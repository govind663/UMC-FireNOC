...
<html lang="en">
    ...
    <head>
        ...
        <title>PMC-Fire NOC | Renew Other NOC List</title>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body" style="border: 1px solid rgb(3, 155, 155);">
                                        ...
                                        @if($status == 0)
                                        <h4 class="card-header text-primary">All Pending Renew Other NOC List</h4>
                                        @elseif($status == 1)
                                        <h4 class="card-header text-primary">All Unpaid Renew Other NOC List</h4>
                                        @elseif($status == 2)
                                        <h4 class="card-header text-primary">All Paid Renew Other NOC List</h4>
                                        @elseif($status == 3)
                                        <h4 class="card-header text-primary">All Approved Renew Other NOC List</h4>
                                        @elseif($status == 4)
                                        <h4 class="card-header text-primary">All Rejected Renew Other NOC List</h4>
                                        @elseif($status == 5)
                                        <h4 class="card-header text-primary">All Underprocess Renew Other NOC List</h4>
                                        @elseif($status == 6)
                                        <h4 class="card-header text-primary">All Reviewed Renew Other NOC List</h4>
                                        @elseif($status == 7)
                                        <h4 class="card-header text-primary">All Invoice Generated Renew Other NOC List</h4>
                                        @endif
                                                ...
                                                @foreach ($data as $key => $value)
                                                    <tr>
                                                        ...
                                                        <td style="display:flex;">
                                                            <a href='{{ url("/renew_other_noc/show/{$value->RB_NOC_ID}/{$value->status}") }}' class="btn btn-primary btn-sm">
                                                                <b><i class="mdi mdi-eye-circle-outline"> View</i></b>
                                                            </a>
                                                            &nbsp;&nbsp;
                                                            <a href='{{ url("/download_renew_other_noc_pdf/{$value->RB_NOC_ID}/{$value->status}") }}' class="btn btn-dark btn-sm ">
                                                                <b><i class="mdi mdi-file"> Download View </i></b>
                                                            </a>

                                                            @if ($value->status == 0 || $value->status == 4)
                                                            &nbsp;&nbsp;
                                                            <a href='{{ url("/renew_other_noc/edit/{$value->RB_NOC_ID}/{$value->status}") }}' class="btn btn-warning btn-sm text-dark">
                                                                <b><i class="mdi mdi-account-edit"> Edit</i></b>
                                                            </a>
                                                            &nbsp;&nbsp;
                                                            <form action='{{ url("/renew_other_noc/delete/{$value->RB_NOC_ID}/{$value->d_ID}/{$value->status}") }}' method="post">
                                                                @csrf
                                                                @method('DELETE')
                                                                        ...
                                                                        <form class="auth-input p-4"  method="POST" action='{{ url("/upload_payment_receipt/{$value->RB_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' enctype="multipart/form-data" autocomplete="off" >
                                                                            ...
                                                                            <div class="form-group row  mb-3">
                                                                                ...
                                                                                <div class="col-sm-10 col-md-10">
                                                                                    <input type="file" accept=".jpg, .jpeg, .png, .pdf" required name="payment_recepit_doc" id="payment_recepit_doc" class="form-control  @error('payment_recepit_doc') is-invalid @enderror " value="{{ old('commissioning_certificate') }}" placeholder="Upload Payment Receipt.">

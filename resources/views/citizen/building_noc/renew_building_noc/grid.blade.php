...
<html lang="en">
    ...
    <head>
        ...
        <title>UMC-Fire NOC | Renew Building NOC List</title>
                            ...
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body" style="border: 1px solid rgb(3, 155, 155);">
                                        ...
                                        @if($status == 0)
                                        <h4 class="card-header text-primary">All Pending Renew Building NOC List</h4>
                                        @elseif($status == 1)
                                        <h4 class="card-header text-primary">All Unpaid Renew Building NOC List</h4>
                                        @elseif($status == 2)
                                        <h4 class="card-header text-primary">All Paid Renew Building NOC List</h4>
                                        @elseif($status == 3)
                                        <h4 class="card-header text-primary">All Approved Renew Building NOC List</h4>
                                        @elseif($status == 4)
                                        <h4 class="card-header text-primary">All Rejected Renew Building NOC List</h4>
                                        @elseif($status == 5)
                                        <h4 class="card-header text-primary">All Underprocess Renew Building NOC List</h4>
                                        @elseif($status == 6)
                                        <h4 class="card-header text-primary">All Reviewed Renew Building NOC List</h4>
                                        @elseif($status == 7)
                                        <h4 class="card-header text-primary">All Invoice Generated Renew Building NOC List</h4>
                                        @endif
                                                ...
                                                @foreach ($data as $key => $value)
                                                    <tr>
                                                        ...
                                                        <td style="display:flex;">
                                                            <a href='{{ url("/renew_building_noc/show/{$value->RB_NOC_ID}/{$value->status}") }}' class="btn btn-primary btn-sm">
                                                                <b><i class="mdi mdi-eye-circle-outline"> View</i></b>
                                                            </a>
                                                            &nbsp;&nbsp;
                                                            <a href='{{ url("/download_renew_building_noc_pdf/{$value->RB_NOC_ID}/{$value->status}") }}' class="btn btn-dark btn-sm ">
                                                                <b><i class="mdi mdi-file"> Download View </i></b>
                                                            </a>
                                                            @if ($value->status == 0 || $value->status == 4)

                                                            &nbsp;&nbsp;
                                                            <a href='{{ url("/renew_building_noc/edit/{$value->RB_NOC_ID}/{$value->status}") }}' class="btn btn-warning btn-sm text-dark">
                                                                <b><i class="mdi mdi-account-edit"> Edit</i></b>
                                                            </a>

                                                            &nbsp;&nbsp;
                                                            <form action='{{ url("/renew_building_noc/delete/{$value->RB_NOC_ID}/{$value->d_ID}/{$value->status}") }}' method="post">
                                                                @csrf
                                                                @method('DELETE')
                                                            ...
                                                            {{-- &nbsp;&nbsp;
                                                            @if ($value->status == 1)
                                                            <a href='{{ url("/make_payment/create/{$value->RB_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' class="btn btn-success btn-sm ">
                                                            ...
                                                            <a href='{{ url("/invoice/{$value->RB_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' class="btn btn-dark btn-sm ">
                                                            ...
                                                            &nbsp;&nbsp;
                                                            @if ($value->status == 3 )
                                                            <a href='{{ url("/certificate/{$value->RB_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' class="btn btn-warning btn-sm text-dark">
                                                            ...
                                                            <button type="button" class="btn btn-warning text-dark btn-sm waves-effect waves-light" data-bs-toggle="modal" data-bs-target=".bs-example-modal-lg_{{ $value->RB_NOC_ID }}">Upload Payment Recepit</button>
                                                            @endif
                                                        </td>
                                                    </tr>

                                                    {{-- Payment Receipt --}}
                                                    <div class="modal fade bs-example-modal-lg_{{ $value->RB_NOC_ID }}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                                                        <div class="modal-dialog modal-lg">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body">
                                                                    <div class="row">
                                                                        <form class="auth-input p-4"  method="POST" action='{{ url("/upload_payment_receipt/{$value->RB_NOC_ID}/{$value->status}/{$value->noc_mode}") }}' enctype="multipart/form-data" autocomplete="off" >
                                                                            @csrf
                                                                                ...
                                                                                <div class="col-sm-2 col-md-2">
                                                                                    <select class="form-control select2 " name="payment_noc_mode" id="payment_noc_mode" type="hidden">
                                                                                        ...
                                                                                        <optgroup label=" ">
                                                                                            ...
                                                                                            <option value="6" {{ $value->noc_mode == "7" ? 'selected' : '' }}>Renew Building NOC</option>
                                                                                        </optgroup>
                                                                                    </select>
                                                                                </div>
                                                                            </div>
                                                                            ...
                                                                            <div class="form-group row  mb-3">
                                                                                ...
                                                                                <div class="col-sm-10 col-md-10">
                                                                                    <input type="file" accept=".jpg, .jpeg, .png, .pdf" required name="payment_recepit_doc" id="payment_recepit_doc" class="form-control  @error('payment_recepit_doc') is-invalid @enderror " value="{{ old('payment_recepit_doc') }}" placeholder="Payment Receipt.">

...
class AdminProvisionalBuildingNOCController extends Controller
{
    ...
    public function index($status)
    {
        if($status == 0 || $status == 5 || $status == 1){
            $query = DB::table('building_noc AS t1')
                    ...
                    ->where('t2.noc_mode', 5) // ==== Provisional NOC (status=1)
                    ->whereNUll('t1.deleted_at')
                    ->whereNUll('t2.deleted_at')
                    ->orderBy('t1.id', 'DESC');
        }else{
            $query = DB::table('building_noc AS t1')
                    ->select('t1.*', 't2.*', 't1.id as P_NOC_ID', 't2.id as d_ID', 't3.citizen_payment_status', 't4.payment_recepit_doc')
                    ->leftJoin('noc_master AS t2', 't2.id', '=', 't1.noc_mst_id')
    ...
    public function approved(Request $request, $id, $status, $auth_role)
    {
        ...
        // display only underprocess form (status=5)
            ...
            if (!empty($request->hasFile('f_inspector_doc'))) {
                ...
                $image->move(public_path('/UMC_FireNOC/Building_NOC/Provisional_BuildingNOC/f_inspector_doc'), $new_name);

                $image_path = "/UMC_FireNOC/Building_NOC/Provisional_BuildingNOC/f_inspector_doc" . $image_name;
                $fileName = $new_name;
            }
        ...
        // display only Reviewed form (status=6)
            ...
            Building_NOC::where('id', $id)->where('status', $status)->update($update);
            return redirect()->route('all_provisional_building_noc_list', 3)->with('message', 'The application form which you had filled for your provisional building noc has been approved Successfully.');
        ...
        // display only Reviewed form (status=6)
            ...
            Building_NOC::where('id', $id)->where('status', $status)->update($update);
            return redirect()->route('all_provisional_building_noc_list', 1)->with('message', 'The application form which you had filled for your provisional hospital noc has been approved Successfully.');
    ...
    public function rejected(RemarksRequest $request, $id, $status, $auth_role)
    {
        ...
        // display only Reviewed form (status=6)
            ...
            Building_NOC::where('id', $id)->where('status', $status)->update($update);
            return redirect()->route('all_provisional_building_noc_list', 4)->with('message', 'The application form which you had filled for your provisional building noc has been rejected Successfully.');
        ...
        // display only Reviewed form (status=6)
            ...
            Building_NOC::where('id', $id)->where('status', $status)->update($update);
            return redirect()->route('all_provisional_building_noc_list', 2)->with('message', 'The application form which you had filled for your new business noc has been rejected Successfully.');
     ...
     * @return \Illuminate\Http\Response
     */
    public function admin_download_provisional_building_noc_pdf($id, $status)
    {
        $data = DB::table('building_noc as t1')
                ->select('t1.*', 't2.*', 't1.id as P_NOC_ID', 't2.id as d_ID')
                ->leftJoin('noc_master as t2', 't2.id', '=', 't1.noc_mst_id' )

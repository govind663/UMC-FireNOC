...
class AdminNewHospitalNOCController extends Controller
{
    ...
    public function index($status)
    {
        if($status == 0 || $status == 5 || $status == 1){
            ...
            $query = DB::table('hospital_noc AS t1')
                    ->select('t1.*', 't2.*', 't1.id as NH_NOC_ID', 't2.id as d_ID', 't3.citizen_payment_status', 't4.payment_recepit_doc')
                    ->leftJoin('noc_master AS t2', 't2.id', '=', 't1.noc_mst_id')
    ...
    public function approved(Request $request, $id, $status, $auth_role)
    {
        ...
        // display only underprocess form (status=5)
            ...
            if (!empty($request->hasFile('f_inspector_doc'))) {
                ...
                $image->move(public_path('/PMC_FireNOC/Hospital_NOC/New_HospitalNOC/f_inspector_doc'), $new_name);

                $image_path = "/PMC_FireNOC/Hospital_NOC/New_HospitalNOC/f_inspector_doc" . $image_name;
                $fileName = $new_name;
            }
     ...
     * @return \Illuminate\Http\Response
     */
    public function admin_download_new_hospital_noc_pdf($id, $status)
    {
        $data = DB::table('hospital_noc as t1')
                ->select('t1.*', 't2.*', 't1.id as NH_NOC_ID', 't2.id as d_ID')
                ->leftJoin('noc_master as t2', 't2.id', '=', 't1.noc_mst_id' )

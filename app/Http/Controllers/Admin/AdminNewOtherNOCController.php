...
class AdminNewOtherNOCController extends Controller
{
    /**
    ...
    public function index($status)
    {
        ...
        if($status == 0 || $status == 5 || $status == 1){
            $query = DB::table('other_noc AS t1')
                    ->select('t1.*', 't2.*', 't1.id as NC_NOC_ID', 't2.id as d_ID')
                    ->leftJoin('noc_master AS t2', 't2.id', '=', 't1.noc_mst_id')
                    ->where('t2.noc_mode', 8) // ==== New Other NOC (status=1)
                    ->whereNUll('t1.deleted_at')
                    ->whereNUll('t2.deleted_at')
                    ->orderBy('t1.id', 'DESC');
        }else{
            $query = DB::table('other_noc AS t1')
                    ->select('t1.*', 't2.*', 't1.id as NC_NOC_ID', 't2.id as d_ID', 't3.citizen_payment_status', 't4.payment_recepit_doc')
                    ->leftJoin('noc_master AS t2', 't2.id', '=', 't1.noc_mst_id')
        ...
        $data = $query->get();
    // return($data);
        return view('admin.other_noc.new_other_noc.grid')->with('data', $data)->with('status', $status);
    }
    ...
    public function show($id, $status)
    {
        ...
        return view('admin.other_noc.new_other_noc.view')->with('data', $data)->with('status', $status);
    }
    ...
    public function approved(Request $request, $id, $status, $auth_role)
    {
        ...
        if (Auth::user()->role == 0) {
            ...
            Other_NOC::where('id', $id)->where('status', $status)->update($update);
            return redirect()->route('all_new_other_noc_list', 1)->with('message', 'The application form which you had filled for your new other noc has been approved Successfully.');
        ...
        // display only underprocess form (status=5)
            ...
            if (!empty($request->hasFile('f_inspector_doc'))) {
                ...
                $image->move(public_path('/PMC_FireNOC/other_noc/New_OtherNOC/f_inspector_doc'), $new_name);

                $image_path = "/PMC_FireNOC/other_noc/New_OtherNOC/f_inspector_doc" . $image_name;
                $fileName = $new_name;
            }
        ...
        // display only Reviewed form (status=6)
            ...
            Other_NOC::where('id', $id)->where('status', $status)->update($update);
            return redirect()->route('all_new_other_noc_list', 3)->with('message', 'The application form which you had filled for your new other noc has been approved Successfully.');
        }
    }
    ...
    public function rejected(RemarksRequest $request, $id, $status, $auth_role)
    {
        ...
        if (Auth::user()->role == 0) {
            ...
            Other_NOC::where('id', $id)->where('status', $status)->update($update);
            return redirect()->route('all_new_other_noc_list', 2)->with('message', 'The application form which you had filled for your new other noc has been rejected Successfully.');
        ...
        // display only Reviewed form (status=6)
            ...
            Other_NOC::where('id', $id)->where('status', $status)->update($update);
            return redirect()->route('all_new_other_noc_list', 4)->with('message', 'The application form which you had filled for your new other noc has been rejected Successfully.');
        }
    }
    ...
    public function list($all_status)
    {
        ...
        $query = DB::table('other_noc AS t1')
                    ->select('t1.*', 't2.*', 't1.id as NC_NOC_ID', 't2.id as d_ID')
                    ->leftJoin('noc_master AS t2', 't2.id', '=', 't1.noc_mst_id')
                    ->where('t2.noc_mode', 1) // ==== New Other NOC (status=1)
                    ->whereNUll('t1.deleted_at')
                    ->whereNUll('t2.deleted_at')
                    ->orderBy('t1.id', 'DESC');
        ...
        return view('admin.other_noc.all_application.new_other_noc.grid')->with('data', $data)->with('all_status', $all_status);
    }
    ...
    public function view($id, $all_status)
    {
        ...
        return view('admin.other_noc.all_application.new_other_noc.view')->with('data', $data)->with('all_status', $all_status);
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function admin_download_new_other_noc_pdf($id, $status)
    {
        $data = DB::table('other_noc as t1')
        ...
        return FacadePdf::loadView('citizen.other_noc.new_other_noc.new_other_noc_pdf', compact('data','status'))->setPaper('a4')->stream("New Other NOC".$data->NC_NOC_ID.".pdf");
    }

    ...
    public function approved(Request $request, $id, $status, $auth_role)
    {
        ...
        // display only underprocess form (status=5)
            ...
            if (!empty($request->hasFile('f_inspector_doc'))) {
                ...
                $image->move(public_path('/PMC_FireNOC/Hospital_NOC/Renew_HospitalNOC/f_inspector_doc'), $new_name);

                $image_path = "/PMC_FireNOC/Hospital_NOC/Renew_HospitalNOC/f_inspector_doc" . $image_name;
                $fileName = $new_name;
            }
